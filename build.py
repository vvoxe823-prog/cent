...
# Server-rendered drawing endpoint. The existing JSON API above remains unchanged.
from drawing_renderer import page as drawing_page

@app.post("/generate-plan-drawing")
def generate_plan_drawing():
    form = request.form
    required_fields = ["project_type","occupancy","floors","units_per_floor","plot_width","plot_depth","unit","facing","road_side","bedrooms","bathrooms","living_rooms","kitchens","dining_rooms","pooja_rooms","utility_rooms","store_rooms","study_rooms","parking_cars","parking_bikes","staircase_type","lift"]
    missing = [field for field in required_fields if not form.get(field)]
    if missing:
        return Response("<h2>Missing required fields</h2><p>Please complete the required fields and try again.</p>",status=400,mimetype="text/html")
    data = {
        "project":{"type":form.get("project_type"),"occupancy":form.get("occupancy"),"floors":form.get("floors"),"units_per_floor":integer(form,"units_per_floor",1)},
        "plot":{"width":number(form,"plot_width"),"depth":number(form,"plot_depth"),"unit":form.get("unit"),"facing":form.get("facing"),"road_side":form.get("road_side")},
        "setbacks":{"front":number(form,"setback_front"),"rear":number(form,"setback_rear"),"left":number(form,"setback_left"),"right":number(form,"setback_right")},
        "rooms":{k:integer(form,k) for k in ["bedrooms","bathrooms","living_rooms","kitchens","dining_rooms","pooja_rooms","utility_rooms","store_rooms","study_rooms"]},
        "preferences":{"master_bedroom_size":form.get("master_bedroom_size",""),"bedroom_size":form.get("bedroom_size",""),"kitchen_size":form.get("kitchen_size",""),"living_size":form.get("living_size","")},
        "circulation":{"parking_cars":integer(form,"parking_cars"),"parking_bikes":integer(form,"parking_bikes"),"staircase_type":form.get("staircase_type"),"staircase_preference":form.get("staircase_preference","auto"),"lift":form.get("lift")},
        "features":form.getlist("features[]"),"notes":form.get("notes","").strip()
    }
    if data["plot"]["width"]<=0 or data["plot"]["depth"]<=0:
        return Response("<h2>Invalid plot</h2><p>Plot width and depth must be greater than zero.</p>",status=400,mimetype="text/html")
    data["buildable_area_preliminary"]=calculate_buildable_area(data)
    plans=generate_layouts(data,max_plans=3)
    return Response(drawing_page(data,plans),mimetype="text/html")
