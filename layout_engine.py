...
def _search_layout(specs, width, depth, fixed, strategy, max_nodes=50000):
    """Feasibility-first corner packing with compact-size fallback."""
    ordered = _room_order(specs, strategy)
    nodes = 0
    solution = None

    def candidates(spec, placed):
        sizes = _candidate_sizes(spec)
        mw, md = spec["min"]
        for pair in ((mw, md), (md, mw)):
            pair = (round(pair[0], 2), round(pair[1], 2))
            if pair not in sizes:
                sizes.append(pair)

        xs = {0.0, round(width, 2)}
        ys = {0.0, round(depth, 2)}
        for r in placed:
            xs.update((round(r.x, 2), round(r.x2, 2)))
            ys.update((round(r.y, 2), round(r.y2, 2)))

        out, seen = [], set()
        for w, d in sizes:
            if w <= 0 or d <= 0 or w > width or d > depth:
                continue
            x_values = {0.0, round(width-w, 2)}
            y_values = {0.0, round(depth-d, 2)}
            for x in xs:
                x_values.update((round(x, 2), round(x-w, 2)))
            for y in ys:
                y_values.update((round(y, 2), round(y-d, 2)))

            for x in x_values:
                for y in y_values:
                    if x < -0.01 or y < -0.01 or x > width-w+0.01 or y > depth-d+0.01:
                        continue
                    rect = Rect(spec["name"], round(max(0,x),2),
                                round(max(0,y),2), w, d, spec["kind"])
                    key = (rect.x, rect.y, rect.width, rect.depth)
                    if key not in seen and _rect_free(rect, placed, gap=0.0):
                        seen.add(key)
                        out.append(rect)
        ...
        def cost(r):
            return (
                abs(r.width-tw) + abs(r.depth-td)
                + _zoning_penalty(r, spec["kind"], width, depth, strategy)
                + r.x*0.01 + r.y*0.01
            )
        out.sort(key=cost)
        return out

    def recurse(index, placed):
        ...
        spec = ordered[index]
        for rect in candidates(spec, placed):
            if recurse(index+1, placed+[rect]):
                return True
        return False

    recurse(0, list(fixed))
    return solution
