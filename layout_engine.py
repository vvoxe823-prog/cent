...
def _candidate(data, strategy):
    ...
    # Fast feasibility check: non-overlapping requested rooms plus fixed
    # parking/stair/lift footprints can never exceed the buildable envelope.
    # This is only an area lower bound; the geometry solver still performs the
    # actual fit/overlap test when the lower bound passes.
    fixed_area = sum(r.area for r in fixed if r.kind != "buffer")
    remaining_area = max(0.0, width * depth - fixed_area)
    if requested_area > remaining_area + 0.01:
        warnings = parking_warnings + [
            f"Requested room target area ({requested_area:.0f} sq ft) exceeds "
            f"the remaining buildable area after fixed spaces ({remaining_area:.0f} sq ft)."
        ]
        return fixed, 0.0, list(dict.fromkeys(warnings)), False
    ...
    if layout is None:
        ...
        if requested_area > remaining_envelope:
            warnings.append(
                f"Requested room target area ({requested_area:.0f} sq ft) exceeds "
                f"the remaining buildable area after fixed spaces ({remaining_envelope:.0f} sq ft)."
            )
