...
def _room_order(specs, strategy):
    ...
    def key(s):
        ...
        if s["kind"] in ("master_bedroom", "bedroom", "living"):
            ...
            hard = -1
        elif s["kind"] in ("bathroom", "utility"):
            hard = 0
...
def _search_layout(specs, width, depth, fixed, strategy, max_nodes=12000):
    """
    Robust rectangle-packing search.

    The previous solver generated a large Cartesian product of unrelated X/Y
    anchors and could exhaust its node budget without ever reaching a valid
    arrangement. This version uses a bottom-left/corner placement strategy:
    every new room is aligned to the edges of existing spaces or the envelope.
    Preferred sizes are tried first, followed by compact sizes down to the
    ...
    def candidates_for(spec, placed):
        sizes = _candidate_sizes(spec)
        # For feasibility, minimum dimensions are important. Keep preferred
        # sizes first for quality, but always expose the minimum-size option.
        mw, md = spec["min"]
        min_pair = (round(mw, 2), round(md, 2))
        min_rot = (round(md, 2), round(mw, 2))
        sizes = [s for s in sizes if s not in (min_pair, min_rot)]
        sizes.extend([min_pair, min_rot])

        coords_x = {0.0}
        coords_y = {0.0}

        for r in placed:
            coords_x.add(round(r.x, 2))
            coords_x.add(round(r.x2, 2))
            coords_y.add(round(r.y, 2))
            coords_y.add(round(r.y2, 2))

        out = []
        seen = set()

        for w, d in sizes:
            if w <= 0 or d <= 0 or w > width + 0.01 or d > depth + 0.01:
                continue

            # Bottom-left corner candidates and their edge-aligned variants.
            xs = set(coords_x)
            ys = set(coords_y)

            for cx in list(coords_x):
                xs.add(round(cx - w, 2))
                xs.add(round(cx, 2))
            for cy in list(coords_y):
                ys.add(round(cy - d, 2))
                ys.add(round(cy, 2))

            # A few central anchors help when the free region is not flush
            # against an existing edge.
            xs.update({
                0.0,
                round(max(0.0, width - w), 2),
                round(max(0.0, (width - w) / 2), 2),
            })
            ys.update({
                0.0,
                round(max(0.0, depth - d), 2),
                round(max(0.0, (depth - d) / 2), 2),
            })

            for x in xs:
                if x < -0.01 or x > width - w + 0.01:
                    continue
                x = round(max(0.0, min(x, width - w)), 2)

                for y in ys:
                    if y < -0.01 or y > depth - d + 0.01:
                        continue
                    y = round(max(0.0, min(y, depth - d)), 2)

                    rect = Rect(spec["name"], x, y, w, d, spec["kind"])
                    if _rect_free(rect, placed, gap=0.0):
        ...
        def candidate_cost(r):
            size_penalty = abs(r.width - tw) + abs(r.depth - td)
            zone_penalty = _zoning_penalty(
                r, spec["kind"], width, depth, strategy
            )

            # Strongly prefer lower/left edge packing. This tends to create
            # coherent partitions and leaves one contiguous residual region.
            edge_penalty = r.x * 0.035 + r.y * 0.025

            # Reward touching an existing edge, rather than floating rooms.
            touching = 0
            for p in placed:
                if (
                    abs(r.x2 - p.x) < 0.01 or
                    abs(r.x - p.x2) < 0.01 or
                    abs(r.y2 - p.y) < 0.01 or
                    abs(r.y - p.y2) < 0.01
                ):
                    touching += 1

            return size_penalty + zone_penalty + edge_penalty - touching * 0.35

        out.sort(key=candidate_cost)
        return out

    # We use a feasibility-first search. Once a complete valid packing is
    # found, keep searching for a better one until the node budget is reached.
    def recurse(index, placed, cost, compacted):
        nonlocal nodes, best
        nodes += 1
        ...
        if index == len(ordered):
            room_area = sum(
                r.area for r in placed if r.kind not in ("parking", "buffer")
            )
            utilization = room_area / (width * depth) if width * depth else 0.0

            final_cost = cost + abs(0.68 - utilization) * 8 + compacted * 1.25

            if best is None or final_cost < best[0]:
                best = (final_cost, list(placed))
            return True

        spec = ordered[index]
        candidates = candidates_for(spec, placed)

        # Limit branching but retain a broad set of geometrically distinct
        # candidates. Minimum-size candidates are deliberately retained.
        for rect in candidates[:140]:
            tw, td = spec["target"]
            compact = int(
                abs(rect.width - tw) > 0.01 or
                abs(rect.depth - td) > 0.01
            )

            # Once a good complete plan exists, prune branches that are already
            # much worse than the best known result.
            next_cost = cost + (
                abs(rect.width - tw) + abs(rect.depth - td)
            ) + _zoning_penalty(
                rect, spec["kind"], width, depth, strategy
            )

            if best is not None and next_cost > best[0] + 35:
                continue
...
def _candidate(data, strategy):
    ...
    # Do NOT reject the layout merely because preferred/target room areas
    # exceed the remaining envelope. Target dimensions are preferences; the
    # solver can compact rooms toward their minimum planning dimensions.
    # The geometry search below is the authoritative feasibility test.
    #
    # This prevents a case such as 595 sq ft of preferred room area versus
    # 546 sq ft remaining from being rejected before compact sizes and
    # alternative arrangements are even attempted.

    # Do not reserve an artificial full-width "parking strip".
    # Parking is a fixed rectangle only where it is actually placed. The
    # remaining parts of the buildable envelope stay available to the room
    # solver, allowing compact side-by-side or rear/side arrangements.
    #
    # This is important because parking does not always occupy the entire
    # front width of a plot. Reserving parking_depth across the full width
    # can incorrectly reduce a 24 x 34 ft envelope to a much smaller "house"
    # envelope and reject otherwise feasible room programs.

    # Build staircase/lift as fixed circulation constraints before rooms.
    # They occupy only their actual footprints inside the full envelope.
    fixed_house = []
    house_y = 0.0
    house_depth = depth
    ...
    # Search the ENTIRE buildable envelope. Parking, staircase and lift are
    # already fixed rectangles, so the solver naturally treats only their
    # actual footprints as occupied. No artificial full-width strip or
    # pre-deducted "usable house area" is created.
    ...
    if layout is None:
        ...
        # A simple area diagnostic is still useful, but it must include the
        # actual fixed footprints rather than inventing a reduced house depth.
        fixed_nonparking_area = sum(
            r.area for r in fixed
            if r.kind not in ("parking", "buffer")
        )
        fixed_parking_area = sum(r.area for r in fixed if r.kind == "parking")
        remaining_envelope = max(
            0.0, width * depth - fixed_nonparking_area - fixed_parking_area
        )
        minimum_area = sum(
            s["min"][0] * s["min"][1] for s in specs
        )
        if minimum_area > remaining_envelope:
            warnings.append(
                f"Requested rooms require at least {minimum_area:.0f} sq ft at "
                f"minimum planning dimensions, but only {remaining_envelope:.0f} "
                f"sq ft remains after fixed spaces."
            )
        elif requested_area > remaining_envelope:
            warnings.append(
                f"Preferred room target area ({requested_area:.0f} sq ft) is above "
                f"the remaining {remaining_envelope:.0f} sq ft, so compact room "
                f"dimensions would be required."
            )
